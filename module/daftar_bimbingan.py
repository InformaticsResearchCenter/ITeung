...
def insertNewBimbinganData(npm, pembimbing1, pembimbing2, judul, tipe_bimbingan):
    db=kelas.dbConnect()
    sql=f"INSERT INTO `wanda`.`bimbingan_data`(`npm`, `pembimbing1`, `pembimbing2`, `tahun_id`, `approval_pembimbing1`, `approval_pembimbing2`, `approval_koordinator1`, `approval_koordinator2`, `judul`, `tipe_bimbingan`) VALUES ('{npm}', '{pembimbing1}', '{pembimbing2}', '{kelas.getTahunID()}', 'false', 'false', 'false', 'false', '{judul}', '{tipe_bimbingan}');"
    with db:
        cur=db.cursor()
        cur.execute(sql)

def updateBimbinganData(npm, pembimbing1, pembimbing2, judul, tipe_bimbingan, tahun_id=kelas.getTahunID()):
    db=kelas.dbConnect()
    sql=f"UPDATE `wanda`.`bimbingan_data` SET `pembimbing1` = '{pembimbing1}', `pembimbing2` = '{pembimbing2}', `judul` = '{judul}', `tipe_bimbingan` = '{tipe_bimbingan}' WHERE `npm` = '{npm}' and tahun_id = '{tahun_id}';"
    with db:
        cur=db.cursor()
        cur.execute(sql)
