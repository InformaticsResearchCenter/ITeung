...
def updateBiayaKHS(npm, tahunid, biaya, percentage):
    db=kelas.dbConnectSiap()
    sql=f"UPDATE simak_trn_khs SET Biaya={biaya}, StatusBayar={percentage} WHERE MhswID={npm} and TahunID={tahunid}"
    with db:
        cur=db.cursor()
        cur.execute(sql)
...
def home():
    if is_logged_in():
        if request.method == 'GET':
            ...
            return render_template('chatbot.html', bot_reply='', user_name=is_logged_in()['name'], login_as="Mas Angga", title="Backyard Boy")
        if request.method == 'POST':
            ...
            if studentphonenumber:
                ...
                phonenumber=None
            if phonenumber == None:
                return render_template('chatbot.html', bot_reply='your phone number is not registered as Politeknik Pos Indonesia Academic', user_name=is_logged_in()['name'], login_as="Backyard Boy", title="Backyard Boy")
            else:
                return render_template('chatbot.html', bot_reply=flask_chatbot.cekAndSendMessage(message, phonenumber), user_name=is_logged_in()['name'], login_as="Backyard Boy", title="Backyard Boy")
    else:
